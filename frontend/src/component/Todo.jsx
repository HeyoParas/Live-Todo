    ...
    <div className='flex flex-col h-full border-2 border-dashed border-black-300 rounded-lg'>
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <Droppable droppableId="todoList">
          {(provided) => (
            <div
              className="h-full w-full overflow-y-auto hide-scrollbar p-1 my-1"
            ...
            >
              {todos.map((todo, index) => (
                ...
                <Draggable key={todo.id} draggableId={todo._id.toString()} index={index}>
                  {(provided) => (
                    <div
                      ref={provided.innerRef}
                      {...provided.draggableProps}
                      {...provided.dragHandleProps}
                      className="h-55 m-2 rounded-xl border border-gray-400"
                    >
